...
public class UserController {
    ...
    public @ResponseBody String callback(String code) throws JsonProcessingException {
        ...
        ResponseEntity<String> tokenEntiy = Fetch.kakao("https://kapi.kakao.com/v2/user/me", HttpMethod.POST, tp.getAccessToken());

        // 6. 해당 email로 회원가입 되어 있는 user 정보가 있는지 DB 조회 (X)
        
        // 7. 있으면 그 user 정보로 session 만들기. (자동로그인) (X)
        
        // 8. 없으면 강제 회원가입 시키고, 그 정보로 session 만들어주기. (자동로그인)

        return tokenEntiy.getBody();
    }
}
